...
const CADENCE_INFO = {
  weekly: {
    ...
    period: "week",
    description:
      "You have a weekly streak going. Keep your streak alive by updating a session each week.",
  },
  biweekly: {
    label: "Biweekly",
    period: "biweekly period",
    description:
      "You have a biweekly streak going. Keep your streak alive by updating a session each two-week window.",
  },
  monthly: {
    label: "Monthly",
    period: "month",
    description:
      "You have a monthly streak going. Keep your streak alive by updating a session each month.",
  },
};
...
function getStreakStatus(campaign) {
  ...
  const nextPeriodEndInclusive = new Date(
    nextPeriodEnd.getTime() - 24 * 60 * 60 * 1000,
  );

  const isExpired = lastPeriod
    ? getNextPeriodStart(
        getPeriodStart(lastPeriod, cadence),
        cadence,
      ).getTime() < currentPeriodStart.getTime()
    : true;
  const streakCount = isExpired ? 0 : (campaign?.streak_count ?? 0);
  ...
  const daysRemaining = Math.ceil(
    (deadlineEnd.getTime() - todayDate.getTime()) / (24 * 60 * 60 * 1000),
  );
