...
export default function ColorPickerModal({ isOpen, onClose, currentColor, onSelectColor }) {
  const headerContent = (
    <div>
      ...
      <p className="text-xs text-slate-500 dark:text-gray-400 mt-0.5 font-normal">Choose your presence and cursor color.</p>
    </div>
  )
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title={headerContent} size="sm">
      <div className="space-y-6">
        ...
        <div className="flex justify-end gap-2.5 pt-6 border-t border-slate-100 dark:border-gray-700/50">
          <Button 
            variant="outline" 
            type="button" 
            onClick={onClose} 
